...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".elebot" / "tool-results" / "test_runner" / "call_big.txt").exists()
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("elebot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "elebot.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("elebot.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from elebot.agent.loop import AgentLoop
    from elebot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from elebot.bus.events import InboundMessage
    from elebot.bus.queue import MessageBus
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from elebot.agent.loop import AgentLoop
    from elebot.agent.runner import _BACKFILL_CONTENT
    from elebot.bus.events import InboundMessage
    from elebot.bus.queue import MessageBus
