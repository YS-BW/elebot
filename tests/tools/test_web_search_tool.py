...
from elebot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="elebot", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("elebot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import elebot.agent.tools.web as web_mod
