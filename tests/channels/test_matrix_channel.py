...
from elebot.channels.matrix import _build_matrix_text_content

import elebot.channels.matrix as matrix_module
from elebot.bus.events import OutboundMessage
from elebot.bus.queue import MessageBus
from elebot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from elebot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("elebot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "elebot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("elebot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "elebot.channels.matrix.asyncio.create_task", _fake_create_task
    )
