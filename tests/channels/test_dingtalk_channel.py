...
try:
    from elebot.channels import dingtalk
...
import elebot.channels.dingtalk as dingtalk_module
from elebot.channels.dingtalk import DingTalkChannel, NanobotDingTalkHandler
from elebot.channels.dingtalk import DingTalkConfig
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/elebot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/elebot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "elebot.config.paths.get_media_dir",
