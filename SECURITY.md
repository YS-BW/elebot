...
If you discover a security vulnerability in elebot, please report it by:
...
chmod 600 ~/.elebot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.elebot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.elebot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run elebot with a dedicated user account
...
- Keep authentication data in `~/.elebot/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for elebot and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install elebot-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash elebot
   sudo -u elebot elebot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.elebot
   chmod 600 ~/.elebot/config.json
   chmod 700 ~/.elebot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.elebot/logs/elebot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.elebot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.elebot/logs/elebot.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/elebot/security/advisories
- Release Notes: https://github.com/HKUDS/elebot/releases
