...
Build a custom elebot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of elebot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

elebot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `elebot gateway` starts, it scans:

1. Built-in channels in `elebot/channels/`
2. External packages registered under the `elebot.channels` entry point group
...
```
elebot-channel-webhook/
├── elebot_channel_webhook/
...
```python
# elebot_channel_webhook/__init__.py
from elebot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# elebot_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "elebot-channel-webhook"
version = "0.1.0"
dependencies = ["elebot", "aiohttp"]

[project.entry-points."elebot.channels"]
webhook = "elebot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
elebot plugins list      # verify "Webhook" shows as "plugin"
elebot onboard           # auto-adds default config for detected plugins
```

Edit `~/.elebot/config.json`:
...
```bash
elebot channels login <channel_name>
elebot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `elebot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `elebot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `elebot.config.schema.Base`:
...
Override `default_config()` so `elebot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `elebot-channel-{name}` | `elebot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `elebot_channel_{name}` | `elebot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/elebot-channel-webhook
cd elebot-channel-webhook
pip install -e .
elebot plugins list    # should show "Webhook" as "plugin"
elebot gateway         # test end-to-end
```
