...
Use elebot programmatically — load config, run the agent, get results.

## Quick Start

```python
import asyncio
from elebot import Elebot
...
### `Elebot.from_config(config_path?, *, workspace?)`

Create an `Elebot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.elebot/config.json`. |
...
```python
from elebot.agent import AgentHook, AgentHookContext
...
async def main():
    bot = Elebot.from_config(workspace="/my/project")
