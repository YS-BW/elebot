...
try:
    ...
    # 中文说明：Fallback so class definitions don't crash at module level
...
class NanobotDingTalkHandler(CallbackHandler):
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            # 中文说明：Parse using SDK's ChatbotMessage for robust handling
            chatbot_msg = ChatbotMessage.from_dict(message.data)

            # 中文说明：Extract text content; fall back to raw dict if SDK object is empty
            content = ""
            if chatbot_msg.text:
            ...
            # 中文说明：Handle file/image messages
            file_paths = []
            ...
            # 中文说明：Forward to Nanobot via _on_message (non-blocking).
            # 中文说明：Store reference to prevent GC before task completes.
            ...
            # 中文说明：Return OK to avoid retry loop from DingTalk server
...
class DingTalkChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # 中文说明：Access Token management for sending messages
        self._access_token: str | None = None
        self._token_expiry: float = 0

        # 中文说明：Hold references to background tasks to prevent GC
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # 中文说明：Register standard handler
            handler = NanobotDingTalkHandler(self)
            self._client.register_callback_handler(ChatbotMessage.TOPIC, handler)

            logger.info("DingTalk bot started with Stream Mode")

            # 中文说明：Reconnect loop: restart stream if SDK exits or crashes
            while self._running:
                try:
    ...
    async def stop(self) -> None:
        ...
        self._running = False
        # 中文说明：Close the shared HTTP client
        if self._http:
            await self._http.aclose()
            self._http = None
        # 中文说明：Cancel outstanding background tasks
    ...
    async def _get_access_token(self) -> str | None:
        ...
        try:
            ...
            # 中文说明：Expire 60s early to be safe
    ...
    async def _send_batch_message(
        ...
        if chat_id.startswith("group:"):
            ...
            payload = {
                ...
                "openConversationId": chat_id[6:],  # 中文说明：Remove "group:" prefix,
                "msgKey": msg_key,
    ...
    async def _send_media_ref(self, token: str, chat_id: str, media_ref: str) -> bool:
        ...
        if upload_type == "image":
            # 中文说明：Verified in production: sampleImageMsg accepts media_id in photoURL.
            ok = await self._send_batch_message(
                token,
                chat_id,
                "sampleImageMsg",
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        for media_ref in msg.media or []:
            ...
            # 中文说明：Send visible fallback so failures are observable by the user.
            filename = self._guess_filename(media_ref, self._guess_upload_type(media_ref))
    ...
    async def _download_dingtalk_file(
        ...
        try:
            ...
            # 中文说明：Step 1: Exchange downloadCode for a temporary download URL
            ...
            # 中文说明：Step 2: Download the file content
            ...
            # 中文说明：Save to media directory (accessible under workspace)
