...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install elebot-ai[matrix]"
    ) from e
...
from elebot.config.paths import get_data_dir, get_media_dir
from elebot.config.schema import Base
from elebot.utils.helpers import safe_filename

TYPING_NOTICE_TIMEOUT_MS = 30_000
# 中文说明：Must stay below TYPING_NOTICE_TIMEOUT_MS so the indicator doesn't expire mid-processing.
...
def _render_markdown_html(text: str) -> str | None:
    ...
    if not formatted:
        return None
    # 中文说明：Skip formatted_body for plain <p>text</p> to keep payload minimal.
...
class MatrixChannel(BaseChannel):
    ...
    name = "matrix"
    display_name = "Matrix"
    _STREAM_EDIT_INTERVAL = 2 # 中文说明：min seconds between edit_message_text calls
    ...
    async def send_delta(self, chat_id: str, delta: str, metadata: dict[str, Any] | None = None) -> None:
        ...
        if not buf.last_edit or (now - buf.last_edit) >= self._STREAM_EDIT_INTERVAL:
            try:
                ...
                buf.last_edit = now
                if not buf.event_id:
                    # 中文说明：we are editing the same message all the time, so only the first time the event id needs to be set
