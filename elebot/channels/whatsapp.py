...
class WhatsAppConfig(Base):
    ...
    group_policy: Literal["open", "mention"] = "open"  # 中文说明："open" responds to all, "mention" only when @mentioned
...
class WhatsAppChannel(BaseChannel):
    ...
    async def _handle_bridge_message(self, raw: str) -> None:
        ...
        if msg_type == "message":
            # 中文说明：Incoming message from WhatsApp
            # 中文说明：Deprecated by whatsapp: old phone number style typically: <phone>@s.whatspp.net
            pn = data.get("pn", "")
            # 中文说明：New LID sytle typically:
            ...
            # 中文说明：Extract just the phone number or lid as chat_id
            ...
            # 中文说明：Classify by JID suffix: @s.whatsapp.net = phone, @lid.whatsapp.net = LID
            # 中文说明：The bridge's pn/sender fields don't consistently map to phone/LID across versions.
            raw_a = pn or ""
            raw_b = sender or ""
            ...
            for raw, extracted in [(raw_a, id_a), (raw_b, id_b)]:
                if "@s.whatsapp.net" in raw:
                    ...
                    phone_id = extracted  # 中文说明：best guess for bare values
            ...
            # 中文说明：Extract media paths (images/documents/videos downloaded by the bridge)
            media_paths = data.get("media") or []

            # 中文说明：Handle voice transcription if it's a voice message
            ...
            # 中文说明：Build content tags matching Telegram's pattern: [image: /path] or [file: /path]
            if media_paths:
            ...
            await self._handle_message(
                sender_id=sender_id,
                chat_id=sender,  # 中文说明：Use full LID for replies
                content=content,
                media=media_paths,
                metadata={
            ...
            # 中文说明：QR code for authentication
...
def _ensure_bridge_setup() -> Path:
    ...
    if not source:
        raise RuntimeError(
            ...
            "Try reinstalling: pip install --force-reinstall elebot"
        )
