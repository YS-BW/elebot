...
from elebot.utils.helpers import safe_filename
...
class EmailConfig(Base):
    ...
    # 中文说明：Email authentication verification (anti-spoofing)
    verify_dkim: bool = True   # 中文说明：Require Authentication-Results with dkim=pass
    verify_spf: bool = True    # 中文说明：Require Authentication-Results with spf=pass

    # 中文说明：Attachment handling — set allowed types to enable (e.g. ["application/pdf", "image/*"], or ["*"] for all)
    allowed_attachment_types: list[str] = Field(default_factory=list)
    max_attachment_size: int = 2_000_000  # 中文说明：2MB per attachment
...
class EmailChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._processed_uids: set[str] = set()  # 中文说明：Capped to prevent unbounded growth
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # 中文说明：Determine if this is a reply (recipient has sent us an email before)
        is_reply = to_addr in self._last_subject_by_chat
        force_send = bool((msg.metadata or {}).get("force_send"))

        # 中文说明：autoReplyEnabled only controls automatic replies, not proactive sends
        if is_reply and not self.config.auto_reply_enabled and not force_send:
            logger.info("Skip automatic email reply to {}: auto_reply_enabled is false", to_addr)
            return

        base_subject = self._last_subject_by_chat.get(to_addr, "elebot reply")
    ...
    def fetch_messages_between_dates(
        ...
        """中文说明：fetch_messages_between_dates。
        ...
        try:
            ...
            for imap_id in ids:
                ...
                if dedupe and uid:
                    ...
                    # 中文说明：mark_seen is the primary dedup; this set is a safety net
                    if len(self._processed_uids) > self._MAX_PROCESSED_UIDS:
                        # 中文说明：Evict a random half to cap memory; mark_seen is the primary dedup
    ...
    def _reply_subject(self, base_subject: str) -> str:
        subject = (base_subject or "").strip() or "elebot reply"
