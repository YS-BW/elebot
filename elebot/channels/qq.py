...
# 中文说明：QQ rich media file_type: 1=image, 4=file
# 中文说明：(2=voice, 3=video are restricted; we only use image vs file)
...
# 中文说明：Replace unsafe characters with "_", keep Chinese and common safe punctuation.
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # 中文说明：Disable botpy's file log — elebot uses loguru; default "botpy.log" fails on read-only fs
            """中文说明：__init__。
        ...
        async def on_group_at_message_create(self, message: GroupMessage):
            """中文说明：on_group_at_message_create。
...
class QQConfig(Base):
    ...
    # 中文说明：Optional: directory to save inbound attachments. If empty, use elebot get_media_dir("qq").
    media_dir: str = ""

    # 中文说明：Download tuning
    download_chunk_size: int = 1024 * 256  # 中文说明：256KB
    download_max_bytes: int = 1024 * 1024 * 200  # 中文说明：200MB safety limit
...
class QQChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._msg_seq: int = 1  # 中文说明：used to avoid QQ API dedup
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".elebot" / "media" / "qq"
        else:
            root = Path.home() / ".elebot" / "media" / "qq"
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            # 中文说明：Network / transport errors — propagate so ChannelManager can retry
            raise
        except Exception:
    ...
    async def _send_media(
        ...
        try:
            ...
            # 中文说明：Network / transport errors — propagate for retry by caller
            logger.warning("QQ send media network error filename={} err={}", filename, e)
            raise
        except Exception as e:
            # 中文说明：API-level or other non-network errors — return False so send() can fallback
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        # 中文说明：Local file: plain path or file:// URI
        if not media_ref.startswith("http://") and not media_ref.startswith("https://"):
            try:
                if media_ref.startswith("file://"):
                    parsed = urlparse(media_ref)
                    # 中文说明：Windows: path in netloc; Unix: path in path
    ...
    # 中文说明：https://github.com/tencent-connect/botpy/issues/198
    # 中文说明：https://bot.q.qq.com/wiki/develop/api-v2/server-inter/message/send-receive/rich-media.html
    async def _post_base64file(
        ...
        # 中文说明：Only pass file_name for non-image types (file_type=4).
        # 中文说明：Passing file_name for images causes QQ client to render them as
        # 中文说明：file attachments instead of inline images.
        ...
        # 中文说明：Extract only the file_info field to avoid extra fields (file_uuid, ttl, etc.)
        # 中文说明：that may confuse QQ client when sending the media object.
        if isinstance(result, dict) and "file_info" in result:
            return {"file_info": result["file_info"]}
        return result

    # ---------------------------
    # 中文说明：Inbound (receive)
    # ---------------------------
    ...
    async def _on_message(self, data: C2CMessage | GroupMessage, is_group: bool = False) -> None:
        ...
        try:
            ...
            # 中文说明：the data used by tests don't contain attachments property
            # 中文说明：so we use getattr with a default of [] to avoid AttributeError in tests
            attachments = getattr(data, "attachments", None) or []
            media_paths, recv_lines, att_meta = await self._handle_attachments(attachments)

            # 中文说明：Compose content that always contains actionable saved paths
            if recv_lines:
                tag = (
                    "[Image]"
    ...
    async def _download_to_media_dir_chunked(
        ...
        """
        # 中文说明：Handle protocol-relative URLs (e.g. "//multimedia.nt.qq.com/...")
        if url.startswith("//"):
            url = f"https:{url}"
        ...
        try:
            async with self._http.get(
                ...
                # 中文说明：Infer extension: url -> filename_hint -> content-type -> fallback
                ...
                tmp_path = None  # 中文说明：mark as moved
