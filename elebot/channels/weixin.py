...
from elebot.utils.helpers import split_message

# ---------------------------------------------------------------------------
# 中文说明：Protocol constants (from openclaw-weixin types.ts)
# ---------------------------------------------------------------------------

# 中文说明：MessageItemType
ITEM_TEXT = 1
ITEM_IMAGE = 2
ITEM_VOICE = 3
ITEM_FILE = 4
...
# 中文说明：MessageType  (1 = inbound from user, 2 = outbound from bot)
MESSAGE_TYPE_USER = 1
MESSAGE_TYPE_BOT = 2
...
# 中文说明：Session-expired error code
ERRCODE_SESSION_EXPIRED = -14
SESSION_PAUSE_DURATION_S = 60 * 60

# 中文说明：Retry constants (matching the reference plugin's monitor.ts)
...
# 中文说明：Default long-poll timeout; overridden by server via longpolling_timeout_ms.
DEFAULT_LONG_POLL_TIMEOUT_S = 35

# 中文说明：Media-type codes for getuploadurl  (1=image, 2=video, 3=file, 4=voice)
UPLOAD_MEDIA_IMAGE = 1
UPLOAD_MEDIA_VIDEO = 2
UPLOAD_MEDIA_FILE = 3
UPLOAD_MEDIA_VOICE = 4

# 中文说明：File extensions considered as images / videos for outbound media
...
class WeixinConfig(Base):
    ...
    token: str = ""  # 中文说明：Manually set token, or obtained via QR login
    state_dir: str = ""  # 中文说明：Default: ~/.elebot/weixin/
    poll_timeout: int = DEFAULT_LONG_POLL_TIMEOUT_S  # 中文说明：seconds for long-poll
...
class WeixinChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._context_tokens: dict[str, str] = {}  # 中文说明：from_user_id -> context_token
    ...
    # ------------------------------------------------------------------
    # 中文说明：HTTP helpers  (matches api.ts buildHeaders / apiFetch)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 中文说明：QR Code Login  (matches login-qr.ts)
    # ------------------------------------------------------------------
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        # 中文说明：Initialize HTTP client for the login flow
        self._client = httpx.AsyncClient(
            timeout=httpx.Timeout(60, connect=30),
            follow_redirects=True,
        )
        self._running = True  # 中文说明：Enable polling loop in _qr_login()
        try:
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'elebot channels login weixin' to authenticate.")
                self._running = False
                return
        ...
        while self._running:
            try:
                ...
                # 中文说明：Normal for long-poll, just retry
                continue
            except Exception:
    ...
    async def stop(self) -> None:
        ...
        self._save_state()
    # ------------------------------------------------------------------
    # 中文说明：Polling  (matches monitor.ts monitorWeixinProvider)
    # ------------------------------------------------------------------
    ...
    async def _poll_once(self) -> None:
        ...
        # 中文说明：Adjust httpx timeout to match the current poll timeout
        assert self._client is not None
        ...
        # 中文说明：Check for API-level errors (monitor.ts checks both ret and errcode)
        ret = data.get("ret", 0)
        ...
        # 中文说明：Honour server-suggested poll timeout (monitor.ts:102-105)
        ...
        # 中文说明：Process messages (WeixinMessage[] from types.ts)
    ...
    # ------------------------------------------------------------------
    # 中文说明：Inbound message processing  (matches inbound.ts + process-message.ts)
    # ------------------------------------------------------------------

    async def _process_message(self, msg: dict) -> None:
        """Process a single WeixinMessage from getUpdates."""
        # 中文说明：Skip bot's own messages (message_type 2 = BOT)
        if msg.get("message_type") == MESSAGE_TYPE_BOT:
            return

        # 中文说明：Deduplication by message_id
        ...
        # 中文说明：Cache context_token (required for all replies — inbound.ts:23-27)
        ctx_token = msg.get("context_token", "")
        if ctx_token:
            self._context_tokens[from_user_id] = ctx_token
            self._save_state()

        # 中文说明：Parse item_list (WeixinMessage.item_list — types.ts:161)
        ...
        for item in item_list:
            ...
            if item_type == ITEM_TEXT:
                ...
                if text:
                    # 中文说明：Handle quoted/ref messages (inbound.ts:86-98)
                    ref = item.get("ref_msg")
                    if ref:
                        ref_item = ref.get("message_item")
                        # 中文说明：If quoted message is media, just pass the text
                ...
                # 中文说明：Voice-to-text provided by WeChat (inbound.ts:101-103)
        ...
        # 中文说明：Fallback: when no top-level media was downloaded, try quoted/referenced media.
        # 中文说明：This aligns with the reference plugin behavior that checks ref_msg.message_item
        # 中文说明：when main item_list has no downloadable media.
    ...
    # ------------------------------------------------------------------
    # 中文说明：Media download  (matches media-download.ts + pic-decrypt.ts)
    # ------------------------------------------------------------------
    ...
    async def _download_media_item(
        ...
        try:
            ...
            # 中文说明：Resolve AES key (media-download.ts:43-45, pic-decrypt.ts:40-52)
            # 中文说明：image_item.aeskey is a raw hex string (16 bytes as 32 hex chars).
            # 中文说明：media.aes_key is always base64-encoded.
            # 中文说明：For images, prefer image_item.aeskey; for others use media.aes_key.
            raw_aeskey_hex = typed_item.get("aeskey", "")
            media_aes_key_b64 = media.get("aes_key", "")

            aes_key_b64: str = ""
            if raw_aeskey_hex:
                # 中文说明：Convert hex → raw bytes → base64 (matches media-download.ts:43-44)
                aes_key_b64 = base64.b64encode(bytes.fromhex(raw_aeskey_hex)).decode()
            elif media_aes_key_b64:
                aes_key_b64 = media_aes_key_b64

            # 中文说明：Reference protocol behavior: VOICE/FILE/VIDEO require aes_key;
            # 中文说明：only IMAGE may be downloaded as plain bytes when key is missing.
    ...
    # ------------------------------------------------------------------
    # 中文说明：Outbound  (matches send.ts buildTextMessageReq + sendMessageWeixin)
    # ------------------------------------------------------------------
        ...
        try:
            ...
            for media_path in (msg.media or []):
                try:
                    ...
                    # 中文说明：Network/transport errors: do NOT fall back to text —
                    # 中文说明：the text send would also likely fail, and the outer
                    # 中文说明：except will re-raise so ChannelManager retries properly.
                    logger.error(
                    ...
                    if status_code >= 500:
                        # 中文说明：Server-side / retryable HTTP error — same as network.
                        logger.error(
                        ...
                        raise
                    # 中文说明：4xx client errors are NOT retryable — fall back to text.
                    ...
                    # 中文说明：Non-network errors (format, file-not-found, etc.):
                    # 中文说明：notify the user via text fallback.
                    filename = Path(media_path).name
                    logger.error("Failed to send WeChat media {}: {}", media_path, e)
                    # 中文说明：Notify user about failure via text
                    await self._send_text(
                        msg.chat_id, f"[Failed to send: {filename}]", ctx_token,
                    )
    ...
    async def _send_media_file(
        ...
        # 中文说明：Determine upload media type from extension
        ext = p.suffix.lower()
        ...
        # 中文说明：Generate client-side AES-128 key (16 random bytes)
        aes_key_raw = os.urandom(16)
        aes_key_hex = aes_key_raw.hex()

        # 中文说明：Compute encrypted size: PKCS7 padding to 16-byte boundary
        # 中文说明：Matches aesEcbPaddedSize: Math.ceil((size + 1) / 16) * 16
        padded_size = ((raw_size + 1 + 15) // 16) * 16

        # 中文说明：Step 1: Get upload URL from server (prefer upload_full_url, fallback to upload_param)
        ...
        # 中文说明：Step 2: AES-128-ECB encrypt and POST to CDN
        ...
        # 中文说明：The download encrypted_query_param comes from CDN response header
        ...
        # 中文说明：Step 3: Send message with the media item
        # 中文说明：aes_key for CDNMedia is the hex key encoded as base64
        # 中文说明：(matches: Buffer.from(uploaded.aeskey).toString("base64"))
        ...
        # 中文说明：Send each media item as its own message (matching reference plugin)
        client_id = f"elebot-{uuid.uuid4().hex[:12]}"
...
# ---------------------------------------------------------------------------
# 中文说明：AES-128-ECB encryption / decryption  (matches pic-decrypt.ts / aes-ecb.ts)
# ---------------------------------------------------------------------------
...
def _parse_aes_key(aes_key_b64: str) -> bytes:
    ...
    * ``base64(hex string of 16 bytes)``  → file / voice / video
    ...
    if len(decoded) == 32 and re.fullmatch(rb"[0-9a-fA-F]{32}", decoded):
        # 中文说明：hex-encoded key: base64 → hex string → raw bytes
