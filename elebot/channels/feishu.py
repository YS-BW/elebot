...
from elebot.config.schema import Base

from lark_oapi.core.const import FEISHU_DOMAIN, LARK_DOMAIN

FEISHU_AVAILABLE = importlib.util.find_spec("lark_oapi") is not None

# 中文说明：Message type display mapping
MSG_TYPE_MAP = {
    "image": "[image]",
    "audio": "[audio]",
    "file": "[file]",
...
def _extract_post_content(content_json: dict) -> tuple[str, list[str]]:
    ...
    # 中文说明：Unwrap optional {"post": ...} envelope
    root = content_json
    ...
    # 中文说明：Localized: prefer known locales, then fall back to any dict child
...
class FeishuConfig(Base):
    ...
    done_emoji: str | None = None  # 中文说明：Emoji to show when task is completed (e.g., "DONE", "OK")
    tool_hint_prefix: str = "\U0001f527"  # 中文说明：Prefix for inline tool hints (default: 🔧)
    group_policy: Literal["open", "mention"] = "mention"
    reply_to_message: bool = False  # 中文说明：If True, bot replies quote the user's original message
    streaming: bool = True
    domain: Literal["feishu", "lark"] = "feishu"  # 中文说明：Set to "lark" for international Lark
...
class FeishuChannel(BaseChannel):
    ...
    _STREAM_EDIT_INTERVAL = 0.5  # 中文说明：throttle between CardKit streaming updates
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._processed_message_ids: OrderedDict[str, None] = OrderedDict()  # 中文说明：Ordered dedup cache
    ...
    async def start(self) -> None:
        ...
        # 中文说明：Create Lark client for sending messages
        ...
        # 中文说明：Create WebSocket client for long connection
        ...
        # 中文说明：Start WebSocket client in a separate thread with reconnect loop.
        # 中文说明：A dedicated event loop is created for this thread so that lark_oapi's
        # 中文说明：module-level `loop = asyncio.get_event_loop()` picks up an idle loop
        # 中文说明：instead of the already-running main asyncio loop, which would cause
        # 中文说明："This event loop is already running" errors.
        def run_ws():
            """中文说明：run_ws。
            ...
            # 中文说明：Patch the module-level loop used by lark's ws Client.start()
        ...
        # 中文说明：Fetch bot's own open_id for accurate @mention matching
        ...
        # 中文说明：Keep running until stopped
        while self._running:
    ...
    def _is_bot_mentioned(self, message: Any) -> bool:
        ...
        for mention in getattr(message, "mentions", None) or []:
            ...
            if self._bot_open_id:
                ...
                # 中文说明：Fallback heuristic when bot open_id is unavailable
    ...
    # 中文说明：Regex to match markdown tables (header + separator + data rows)
    ...
    # 中文说明：Markdown formatting patterns that should be stripped from plain-text
    # 中文说明：surfaces like table cells and heading text.
    ...
    # 中文说明：── Smart format detection ──────────────────────────────────────────
    # 中文说明：Patterns that indicate "complex" markdown needing card rendering
    _COMPLEX_MD_RE = re.compile(
        r"```"  # 中文说明：fenced code block
        r"|^\|.+\|.*\n\s*\|[-:\s|]+\|"  # 中文说明：markdown table (header + separator)
        r"|^#{1,6}\s+",  # 中文说明：headings
        re.MULTILINE,
    )

    # 中文说明：Simple markdown patterns (bold, italic, strikethrough)
    _SIMPLE_MD_RE = re.compile(
        r"\*\*.+?\*\*"  # 中文说明：**bold**
        r"|__.+?__"  # 中文说明：__bold__
        r"|(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)"  # 中文说明：*italic* (single *)
        r"|~~.+?~~",  # 中文说明：~~strikethrough~~
        re.DOTALL,
    )

    # 中文说明：Markdown link: [text](url)
    _MD_LINK_RE = re.compile(r"\[([^\]]+)\]\((https?://[^\)]+)\)")

    # 中文说明：Unordered list items
    _LIST_RE = re.compile(r"^[\s]*[-*+]\s+", re.MULTILINE)

    # 中文说明：Ordered list items
    _OLIST_RE = re.compile(r"^[\s]*\d+\.\s+", re.MULTILINE)

    # 中文说明：Max length for plain text format
    _TEXT_MAX_LEN = 200

    # 中文说明：Max length for post (rich text) format; beyond this, use card
    _POST_MAX_LEN = 2000
    ...
    def _detect_msg_format(cls, content: str) -> str:
        ...
        # 中文说明：Complex markdown (code blocks, tables, headings) → always card
        if cls._COMPLEX_MD_RE.search(stripped):
            return "interactive"

        # 中文说明：Long content → card (better readability with card layout)
        if len(stripped) > cls._POST_MAX_LEN:
            return "interactive"

        # 中文说明：Has bold/italic/strikethrough → card (post format can't render these)
        if cls._SIMPLE_MD_RE.search(stripped):
            return "interactive"

        # 中文说明：Has list items → card (post format can't render list bullets well)
        if cls._LIST_RE.search(stripped) or cls._OLIST_RE.search(stripped):
            return "interactive"

        # 中文说明：Has links → post format (supports <a> tags)
        if cls._MD_LINK_RE.search(stripped):
            return "post"

        # 中文说明：Short plain text → text format
        if len(stripped) <= cls._TEXT_MAX_LEN:
            return "text"

        # 中文说明：Medium plain text without any formatting → post format
        return "post"
    ...
    def _markdown_to_post(cls, content: str) -> str:
        ...
        for line in lines:
            ...
            # 中文说明：Remaining text after last link
            remaining = line[last_end:]
            if remaining:
                elements.append({"tag": "text", "text": remaining})

            # 中文说明：Empty line → empty paragraph for spacing
            if not elements:
    ...
    def _download_image_sync(
        ...
        try:
            ...
            if response.success():
                ...
                # 中文说明：GetMessageResourceRequest returns BytesIO, need to read bytes
    ...
    def _download_file_sync(
        ...
        # 中文说明：Feishu resource download API only accepts 'image' or 'file' as type.
        # 中文说明：Both 'audio' and 'media' (video) messages use type='file' for download.
    ...
    async def _download_and_save_media(
        ...
        if msg_type == "image":
            ...
            # 中文说明：Feishu voice messages are opus in OGG container.
            # 中文说明：Use .ogg extension for better Whisper compatibility.
            if msg_type == "audio":
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            if (message_id := meta.get("message_id")) and (reaction_id := meta.get("reaction_id")):
                ...
                # 中文说明：Add completion emoji if configured
                if self.config.done_emoji and message_id:
                    await self._add_reaction(message_id, self.config.done_emoji)

            resuming = meta.get("_resuming", False)
            if resuming:
                # 中文说明：Mid-turn pause (e.g. tool call between streaming segments).
                # 中文说明：Flush current text to card but keep the buffer alive so the
                # 中文说明：next segment appends to the same card.
            ...
            if buf.card_id:
                ...
                # 中文说明：Required so the chat list preview exits the streaming placeholder (Feishu streaming card docs).
                buf.sequence += 1
                await loop.run_in_executor(
                    None,
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            # 中文说明：Handle tool hint messages.  When a streaming card is active for
            # 中文说明：this chat, inline the hint into the card instead of sending a
            # 中文说明：separate message so the user experience stays cohesive.
            if msg.metadata.get("_tool_hint"):
                ...
                if buf and buf.card_id:
                    # 中文说明：Delegate to send_delta so tool hints get the same
                    # 中文说明：throttling (and card creation) as regular text deltas.
            ...
            # 中文说明：Determine whether the first message should quote the user's message.
            # 中文说明：Only the very first send (media or text) in this call uses reply; subsequent
            # 中文说明：chunks/media fall back to plain create to avoid redundant quote bubbles.
            reply_message_id: str | None = None
            if self.config.reply_to_message and not msg.metadata.get("_progress", False):
                reply_message_id = msg.metadata.get("message_id") or None
            # 中文说明：For topic group messages, always reply to keep context in thread
            elif msg.metadata.get("thread_id"):
                reply_message_id = (
                    msg.metadata.get("root_id") or msg.metadata.get("message_id") or None
                )

            first_send = True  # 中文说明：tracks whether the reply has already been used
            ...
            def _do_send(m_type: str, content: str) -> None:
                ...
                if reply_message_id and first_send:
                    ...
                    if ok:
                        return
                    # 中文说明：Fall back to regular send if reply fails
            ...
            for file_path in msg.media:
                ...
                if ext in self._IMAGE_EXTS:
                    ...
                    if key:
                        # 中文说明：Use msg_type "audio" for audio, "video" for video, "file" for documents.
                        # 中文说明：Feishu requires these specific msg_types for inline playback.
                        # 中文说明：Note: "media" is only valid as a tag inside "post" messages, not as a standalone msg_type.
            ...
            if msg.content and msg.content.strip():
                ...
                if fmt == "text":
                    # 中文说明：Short plain text – send as simple text message
                    text_body = json.dumps({"text": msg.content.strip()}, ensure_ascii=False)
                    await loop.run_in_executor(None, _do_send, "text", text_body)

                elif fmt == "post":
                    # 中文说明：Medium content with links – send as rich-text post
                    post_body = self._markdown_to_post(msg.content)
                    await loop.run_in_executor(None, _do_send, "post", post_body)

                else:
                    # 中文说明：Complex / long content – send as interactive card
    ...
    async def _on_message(self, data: P2ImMessageReceiveV1) -> None:
        ...
        try:
            ...
            if msg_type == "text":
                ...
                if text:
                    content_parts.append(text)
                # 中文说明：Download images embedded in post
                ...
                # 中文说明：Handle share cards and interactive messages
            ...
            # 中文说明：Extract reply context (parent/root message IDs)
            parent_id = getattr(message, "parent_id", None) or None
            root_id = getattr(message, "root_id", None) or None
            thread_id = getattr(message, "thread_id", None) or None

            # 中文说明：Prepend quoted message text when the user replied to another message
    ...
    async def _send_tool_hint_card(
        ...
        # 中文说明：Put each top-level tool call on its own line without altering commas inside arguments.
