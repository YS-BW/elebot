...
# 中文说明：Upload safety limits (matching QQ channel defaults)
WECOM_UPLOAD_MAX_BYTES = 1024 * 1024 * 200  # 中文说明：200MB
...
class WecomChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # 中文说明：Store frame headers for each chat to enable replies
    ...
    async def start(self) -> None:
        ...
        if not WECOM_AVAILABLE:
            logger.error("WeCom SDK not installed. Run: pip install elebot-ai[wecom]")
            return
        ...
        # 中文说明：Create WebSocket client
        self._client = WSClient({
            "bot_id": self.config.bot_id,
            "secret": self.config.secret,
            "reconnect_interval": 1000,
            "max_reconnect_attempts": -1,  # 中文说明：Infinite reconnect
            "heartbeat_interval": 30000,
        })

        # 中文说明：Register event handlers
    ...
    async def _on_enter_chat(self, frame: Any) -> None:
        ...
        try:
            # 中文说明：Extract body from WsFrame dataclass or dict
    ...
    async def _process_message(self, frame: Any, msg_type: str) -> None:
        ...
        try:
            ...
            # 中文说明：Ensure body is a dict
            if not isinstance(body, dict):
                logger.warning("Invalid body type: {}", type(body))
                return

            # 中文说明：Extract message info
            ...
            # 中文说明：Extract sender info from "from" field (SDK format)
            from_info = body.get("from", {})
            sender_id = from_info.get("userid", "unknown") if isinstance(from_info, dict) else "unknown"

            # 中文说明：For single chat, chatid is the sender's userid
            # 中文说明：For group chat, chatid is provided in body
            ...
            if msg_type == "text":
                ...
                # 中文说明：Voice message already contains transcribed content from WeCom
                ...
                # 中文说明：Mixed content contains multiple message items
            ...
            # 中文说明：Store frame for this chat to enable replies
    ...
    async def _upload_media_ws(
        ...
        try:
            ...
            # 中文说明：Read file size and data in a thread to avoid blocking the event loop
            def _read_file():
            ...
            # 中文说明：MD5 is used for file integrity only, not cryptographic security
            md5_hash = hashlib.md5(data).hexdigest()

            CHUNK_SIZE = 512 * 1024  # 中文说明：512 KB raw (before base64)
            mv = memoryview(data)
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            # 中文说明：Get the stored frame for this chat
            frame = self._chat_frames.get(msg.chat_id)

            # 中文说明：Send media files via WebSocket upload
            ...
            if frame:
                # 中文说明：Both progress and final messages must use reply_stream (cmd="aibot_respond_msg").
                # 中文说明：The plain reply() uses cmd="reply" which does not support "text" msgtype
                # 中文说明：and causes errcode=40008 from WeCom API.
                ...
                # 中文说明：No frame (e.g. cron push): proactive send only supports markdown
                await self._client.send_message(msg.chat_id, {
                    "msgtype": "markdown",
